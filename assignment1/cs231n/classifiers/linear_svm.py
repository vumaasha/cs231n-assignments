...
def svm_loss_vectorized(W, X, y, reg):
  ...
  sample_loss = margins.sum(axis=0)
  loss = sample_loss.sum() / num_train
  loss += reg * np.sum(W * W)
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  ...
  # loss.                                                                     #
  #############################################################################
  dsample_loss = 1/num_train
  dscores = (margins > 0).astype(float)
  dscores[range(0,num_train),y] = (dscores.sum(axis=1)*-1)
  dscores = dscores * dsample_loss
